...
class AudioManager {
    ...
    private urlLocks: Map<string, Promise<string>> = new Map();
    ...
    private async getSignedUrl(storagePath: string): Promise<string> {
        ...
        let lock = this.urlLocks.get(storagePath);
        if (lock) {
            return lock; // Return the promise for the pending request
        }

        // Create a new lock promise
        const fetchPromise = this.fetchSignedUrl(storagePath);
    ...
    private async fetchSignedUrl(storagePath: string): Promise<string> {
        try {
