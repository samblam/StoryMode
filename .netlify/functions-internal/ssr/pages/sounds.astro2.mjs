/* empty css                                 */
import { c as createComponent, r as renderTemplate, m as maybeRenderHead, a as addAttribute, d as renderComponent, b as createAstro } from '../chunks/astro/server_Dmwxy-zN.mjs';
import 'kleur/colors';
import 'html-escaper';
import { $ as $$Layout } from '../chunks/Layout_B9cyLK92.mjs';
import { a as $$SoundList, $ as $$ProfileCard } from '../chunks/ProfileCard_ehoV_qc-.mjs';
...
Delete Profile
</button>`} </div> </div> ${sounds.length > 0 ? renderTemplate`${renderComponent($$result, "SoundList", $$SoundList, { "sounds": sounds, "profileId": profileId, "isAdmin": isAdmin })}` : renderTemplate`<p class="text-gray-600 italic">No sounds added to this profile yet.</p>`} </div> `;
...
Sound Profiles
  ...
  }, "isAdmin": isAdmin, "data-astro-cid-ztl5yhde": true })}`)} </div>`)} </div> </div> </div> </main> ` })}  `;
